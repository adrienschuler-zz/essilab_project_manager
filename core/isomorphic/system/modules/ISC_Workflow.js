...
return _2}
,isc.A.$87n=function isc_c_XORGateway__processFieldsRecursively(_1){var _2=[];var _3=isc.XORGateway.$90p(_1);for(var i=0;i<_3.length;i++){_2.add({name:_3[i]})}
return _2}
);isc.B._maxIndex=isc.C+2;isc.A=isc.XORGateway.getPrototype();isc.B=isc._allFuncs;isc.C=isc.B._maxIndex;isc.D=isc._funcClasses;isc.D[isc.C]=isc.A.Class;isc.B.push(isc.A.executeElement=function isc_XORGateway_executeElement(_1){var _2=isc.XORGateway.$87n(this.criteria);var _3=isc.DataSource.create({fields:_2});if(_3.applyFilter([_1.state],this.criteria).length==1){if(this.nextElement)_1.setNextElement(this.nextElement)}else{if(this.failureElement)_1.setNextElement(this.failureElement)}
return true}
...
,isc.A.completeEditing=function isc_UserTask_completeEditing(){if(this.process){var _1;if(this.targetVMValue){_1=this.targetVMValue.getValues()}else if(this.targetFormValue){_1=this.targetFormValue.getValues()}
var _2=this.process;delete this.process;_2.state[this.inputField]=_1;_2.start()}}
,isc.A.executeElement=function isc_UserTask_executeElement(_1){this.process=_1;if(this.targetView&&isc.isA.String(this.targetView)){if(_1.state[this.targetView]){this.targetViewValue=_1.state[this.targetView]}else{this.targetViewValue=window[this.targetView]}}else{this.targetViewValue=this.targetView}
if(this.targetVM&&isc.isA.String(this.targetVM)){if(_1.state[this.targetVM]){this.targetVMValue=_1.state[this.targetVM]}else{this.targetVMValue=window[this.targetVM]}}else{this.targetVMValue=this.targetVM}
if(this.targetForm&&isc.isA.String(this.targetForm)){if(_1.state[this.targetForm]){this.targetFormValue=_1.state[this.targetForm]}else{this.targetFormValue=window[this.targetForm]}}else{this.targetFormValue=this.targetForm}
...
this.targetViewValue.showRecursively();if(this.targetVMValue){this.targetVMValue.setValues(isc.clone(_1.state[this.inputField]));this.targetVMValue.userTask=this}
if(this.targetFormValue){this.targetFormValue.setValues(isc.clone(_1.state[this.inputField]));this.targetFormValue.userTask=this}
return false}
